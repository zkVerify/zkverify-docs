...
Kurier gives you a status flow: `Queued → Valid → Submitted → IncludedInBlock → Finalized` or `Failed`. On aggregation flows, you may later also see `AggregationPending` and `Aggregated`. Current Kurier docs explicitly tie those two aggregation statuses to `chainId`, so do not overgeneralize that rule to every status in the flow.
