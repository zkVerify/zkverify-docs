...
**Cause**: one common cause is expecting aggregation-level status updates without providing `chainId`. Current Kurier docs explicitly mark `AggregationPending` and `Aggregated` as chain-ID-dependent. Another common cause is polling the wrong job or using the wrong network environment.

**Fix**:

- If you need aggregation-level status updates, ensure the `submit-proof` request includes `chainId`.
