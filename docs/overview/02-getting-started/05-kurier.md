---
title: Verifying proofs with Kurier
---
...
In this tutorial, we will be exploring the process of verifying proofs on zkVerify using Kurier. Kurier is a REST API service built by [Horizen Labs](https://horizenlabs.io) which makes the process of verifying proofs on zkVerify very easy and straightforward.
...
Let's create a `.env` file to store our `API_KEY`, which will be used later to send proofs for verification using Kurier. Use the following code snippet to fill up your `.env` file. To use Kurier you need an `API Key`.
Create your own API key by signing up [here for mainnet](https://kurier.xyz) or [here for testnet](https://testnet.kurier.xyz).
...
After sending the verification request to Kurier, we can fetch the status of our request using the ``jobId`` returned in the response of the previous API call. To get the status, we will be making a ``GET`` API call to ``job-status`` endpoint. We want to wait till our proof is finalized on zkVerify, thus we will run a loop waiting for 5 seconds between multiple API calls.
...
3. Reach out to us on [Discord](https://discord.gg/zkverify) or [kurier-support@horizenlabs.io](mailto:kurier-support@horizenlabs.io) if you like to discuss potential partnerships
