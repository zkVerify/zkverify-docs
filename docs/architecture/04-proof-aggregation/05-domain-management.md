...
To put the domain on `Hold` the owner should use the [`holdDomain(domainId)`](../02-mainchain/04-mainchain_api.md#holddomain) extrinsic that emits a [`DomainStateChanged{id, state}`](../02-mainchain/04-mainchain_api.md#domainstatechanged) event, putting the state on `Hold` if there are aggregations that are not published yet, or `Removable` otherwise. If the domain is in `Hold` state, every time that a new aggregation in this domain is published via the `aggregate` extrinsic, the state can change and a new [`DomainStateChanged{id, state}`](../02-mainchain/04-mainchain_api.md#domainstatechanged) can be emitted with the `Removable` state. Remember that if the domain is configured to accept proof only from an allow-list of address the domain still remain in `Hold` state till the list is not empty: use [`removeProofSubmitters`](../02-mainchain/04-mainchain_api.md#removeproofsubmitters) to remove the address reported in [`SubmittersAllowlist` storage](../02-mainchain/04-mainchain_api.md#submittersallowlist) for this domain.
...
For the addresses of the contracts we have deployed on all the destination chains, please refer to the appropriate [section](../08-contract-addresses.md) of the docs.
