...
| **Noir UltraHonk**  | Noir v1.0.0-beta.14 to v1.0.0-beta.18, bb and bb.js v3.0.x (v4.0.0 and later not supported yet), both ZK and non-ZK variants, Keccak256 only (`evm` / `evm-no-zk` verifier target). Submission variants: `V0_84`, `V3_0`, and `Legacy` (backward-compatible alias for `V0_84` that reproduces the pre-versioning statement hash). | **Max number of Public Inputs:** 32, **Max Evaluation Domain Size:** $2^{25}$                                                                                |
