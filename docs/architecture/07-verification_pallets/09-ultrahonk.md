...
| `V3_0`   | `bb` v3.0.x | `keccak256(SCALE(vk))`   | `sha256("ultrahonk:v3.0")`     | New integrations on the current `bb` toolchain.                                                                                                                                                            |
...
1. both the ZK and the Plain (non-ZK) flavors of proofs generated by `bb` are supported;
2. only Keccak256 is supported as the hash function for transcript generation, so proofs and verification keys
   must be generated with the `evm` (ZK) or `evm-no-zk` (Plain) verifier target (`-t` option of `bb`,
   `verifierTarget` option of `bb.js`). Without it, `bb` defaults to the `noir-recursive` target, producing a
   3680-byte `vk` instead of the expected 1888-byte one;
3. proofs generated with `bb`/`bb.js` v4.0.0 or later are not supported yet;
4. recursion is not supported.

- `verify_proof()` uses the [ultrahonk_verifier](https://github.com/zkVerify/ultrahonk_verifier/tree/v0.3.2) crate to verify proofs.
