...
sidebar_position: 1
---
这一章是给排错用的。流程卡在验证事件、聚合 receipt、合约验证或者状态推进时，都可以直接从这里开始查。
...
> 💡 提示：排错时先确认你监听的事件是否正确。监听错事件是最常见的“伪故障”。

> ⚠️ 注意：不要把验证失败和聚合失败混为一谈。验证失败是 proof 本身不通过；聚合失败通常是 domain 或权限问题。

下一节先列最常见的错误模式，以及每种情况应该先查什么。
