...
sidebar_position: 5
---
这一页整理了四个最常见的新手问题。它不是百科，而是帮你尽快做出正确工程判断的速查页。每个问题都按“场景 → 选择 → 后果”展开，方便你快速扫读。
...
> 💡 提示：如果你从 verify‑only 迁移到链上消费，先把 domain 和 receipt 流程跑通，再改业务逻辑。顺序反了会让你反复返工。

> ⚠️ 注意：把 `CannotAggregate` 当成验证失败是最常见的误判。它只是“没进入聚合”，不是 proof 不通过。
