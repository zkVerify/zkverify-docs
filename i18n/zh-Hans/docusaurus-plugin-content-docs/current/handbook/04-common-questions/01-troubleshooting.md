...
> ⚠️ 注意：验证失败不会自动回滚费用，失败也会收费。不要用“盲重试”代替排错。
...
> 💡 提示：聚合失败不是验证失败。你必须区分这两类问题，否则会把问题修错方向。
...
**原因**：一种常见原因是你期待看到聚合层级的状态更新，但提交时没有带 `chainId`。当前 Kurier 官方文档明确把 `AggregationPending` 和 `Aggregated` 标成依赖 `chainId` 的状态。另一种常见原因是轮询了错误的 job，或 API Key / 网络环境不匹配。

**处理方式**：

- 如果你需要聚合层级的状态更新，确保 `submit-proof` 请求包含 `chainId`。
...
> 💡 提示：排错时先定位“验证失败”还是“聚合失败”。这一步能减少 80% 的无效调试。
