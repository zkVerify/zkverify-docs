...
sidebar_position: 1
---
如果你已经理解 ZK 的基本原理，这一条路径关心的是另一件事：**proof 进入 zkVerify 之后，后续到底发生什么，哪些部分仍然是你的系统要负责的**。很多接入问题不是 proving 算法错了，而是工作被分配到了错误的层。
...
> 💡 提示：如果你已经能稳定跑通 proof 生成，但总是在“结果怎么用”这一步卡住，问题往往不在 proving，而在“消费层是否走了聚合发布路径”。

> ⚠️ 注意：不要把 zkVerify 当成 proving 平台。它只负责验证，不会替你生成 proof，也不会帮你决定业务逻辑如何消费结果。
...
这三个问题足够帮你判断自己现在处在哪一层。下一节会从 proof 提交流程开始，把验证层内部的机制一层层拆开。
