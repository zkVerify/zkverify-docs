...
Kurier 会给你一个状态流：`Queued → Valid → Submitted → IncludedInBlock → Finalized` 或 `Failed`。如果你走的是聚合路径，后面还可能看到 `AggregationPending` 和 `Aggregated`。当前 Kurier 官方文档明确把这两个聚合状态和 `chainId` 绑定，所以不要把这个规则过度推广到整条状态流里的每一个状态。
...
> 💡 提示：你可以先用 Kurier 跑通流程，再把关键路径迁移到主链交互。这样既能快速验证思路，又能保留后期的控制权。

> ⚠️ 注意：不要把 Kurier 当成“绕过链上逻辑”的捷径。它只是把链上流程封装了，你仍然要承担链上规则与结果的一致性。
