...
**Domain 出现在哪里**
当你选择聚合路径时，你会在几类提交接口里明确看到 domainId：Kurier、zkVerifyJS 和 PolkadotJS 的 `submitProof` 调用都会暴露这个参数。对 verify-only 来说它是可省略的；但一旦你要进入聚合，它就会变成决定 proof 后续是否进入聚合的关键路由参数。系统在验证之后会检查 domain，如果 domainId 不提供，就不会做任何聚合动作。
...
> ⚠️ 注意：`CannotAggregate` 不是验证失败，它是“聚合失败”。如果你把它当作验证失败，会误判问题根因。

> 💡 提示：如果你打算做链上消费，先确认 domain 存在且处于 Ready，再提交 proof。不要等到发现没有 receipt 才回头找原因。
