...
如果你走 Kurier 路线，验证相关的状态流通常表现为：`Queued` → `Valid` → `Submitted` → `IncludedInBlock` → `Finalized` 或 `Failed`。如果你走的是聚合路径，后面还可能出现 `AggregationPending` → `Aggregated`。当前 Kurier 官方文档明确把 `AggregationPending` 和 `Aggregated` 标为依赖 `chainId` 的状态，所以不要把整条状态流都简化成“没有 chainId 就完全没有状态”。

> ⚠️ 注意：只要你走链上验证，失败交易仍会被打包并收费。不要用“无限重试”来当作兜底策略。

> 💡 提示：如果你既要 verify-only 也要可能进入聚合，就同时监听 `ProofVerified` 与 `CannotAggregate`，否则你会误判 proof 的最终路径。
