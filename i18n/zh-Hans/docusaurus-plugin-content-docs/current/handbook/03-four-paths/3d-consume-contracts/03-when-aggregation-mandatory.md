...
sidebar_position: 4
---
这一页讲清楚“聚合什么时候只是可选，什么时候已经变成必需”。核心判断很简单：如果消费端在链上，合约需要 receipt 才能验证，所以你必须走聚合；如果消费端只在应用侧，你可以停在验证结果上。
...
> ⚠️ 注意：不要把“验证通过”当成“链上可用”。合约只接受 receipt + Merkle path。

> 💡 提示：如果你不确定消费端是否会上链，先按 verify‑only 设计；一旦确定要链上消费，再切入聚合路径。
