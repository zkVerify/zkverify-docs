...
sidebar_position: 2
---
这一节写给已经有合约的接入场景。这里最关键的问题不是 proof 存不存在，而是**合约靠什么数据来信任这次验证结果**。答案是 receipt 加 Merkle path，而不是原始 proof。所以这条路径一定会经过 verify + aggregate。
...
> ⚠️ 注意：如果你不记录 receipt 事件所在的 block hash，后续无法计算 Merkle path。

> 💡 提示：合约验证失败时，先检查 leafIndex/numberOfLeaves 是否与 receipt 对应，顺序错了会直接失败。

先记住这条边界：**合约端消费的是 receipt + Merkle path，不是 proof 本身**。下一节会讲没有合约时如何消费验证结果。
