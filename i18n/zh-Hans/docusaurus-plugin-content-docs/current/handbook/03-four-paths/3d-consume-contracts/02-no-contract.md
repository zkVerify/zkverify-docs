...
在 verify‑only 路线里，你的核心信号是“稳定的验证结果”。一种常见实现方式是用 Kurier 的 `job-status` 轮询：状态从 `Submitted` → `IncludedInBlock` → `Finalized`，当它进入 `Finalized`，你就可以认为验证结果稳定可用。
...
这里有一个更准确的注意点：当前 Kurier 官方文档明确把 `AggregationPending` 和 `Aggregated` 标成依赖 `chainId` 的状态。如果你的流程需要聚合层级的状态更新，就要确保提交 proof 时带上 `chainId`；但不要把这个规则直接套到前面的所有验证状态上。
...
> 💡 提示：先把 `Finalized` 作为默认门槛，如果你需要更快的反馈，再考虑 `IncludedInBlock`。

> ⚠️ 注意：不要把 `Submitted` 当成验证成功。它只是“已经送上链”，不是“结果已稳定”。
