...
当你切换证明系统时，最容易踩的坑不是“语法不一样”，而是 setup 模式完全不同。setup 会决定你能否复用 vk、是否需要可信仪式、以及你要把哪些产物存到哪里。这就是为什么这页要把 Groth16、Noir 和 zkVM 拆开讲。

可以把 setup 理解成“工厂的校准流程”。有的系统每条产线都要重新校准（per-circuit），有的系统用一套通用校准（universal SRS），还有的系统干脆不需要校准（transparent）。你如果把这三类混在一起，最常见的后果就是 vk 无法复用，或者你以为不需要 setup 但实际上缺了必要产物。
...
> 📌 Note: 这里的“可复用性”指 vk 和 setup 产物是否能跨电路复用，不是指 proof 本身。
