...
可以把这一过程理解成“海关验收单”。zkVerify 给你一张验收单（receipt），合约只看验收单来放行。你如果没有验收单，即使 proof 真实有效，合约也不会信你。
...
> ⚠️ Warning: 只提交 proof 不会产生合约可用结果。没有 receipt，合约端无法验证你的 proof 被 zkVerify 接受。

> 💡 Tip: 如果你只是 Web2 侧消费，先不要引入合约流程。verify-only 能让你更快上线，再决定是否进入聚合路径。

这一节是合约消费路径的入口。下一节会从“我有合约”开始，把 end‑to‑end 流程跑通，再往下才会讲“没有合约时怎么消费”。
