...
## Example 1: Membership + nullifier（防止重复使用）
...
> 💡 Tip: nullifier 的本质是“可公开的一次性标识”，它必须进 public inputs 才有意义。

---

## Example 2: Proof bound to action（防重放）
...
> ⚠️ Warning: proof 如果不绑定动作，就等于可重放凭证，任何人拿到都能重复使用。

---

## Example 3: Web2 verify‑only template（验证结果持久化）
...
> 💡 Tip: statement 是验证结果的唯一指纹，把它当作审计索引来用。
...
这三类例子解决的都是“真实系统里的坑”，而不是“能不能跑通”。你跑完示例之后，应该明确三件事：

1) 你的 proof 是否绑定了唯一性或动作语义；
2) 你的验证结果是否能被审计与复用；
3) 你的消费逻辑是否能区分“验证通过”和“业务完成”。

如果你能回答这三个问题，就意味着你已经从“演示级”走到“可上线级”。下一节会给出统一的示例模板，帮助你把这些模式迁移到新的用例里。
