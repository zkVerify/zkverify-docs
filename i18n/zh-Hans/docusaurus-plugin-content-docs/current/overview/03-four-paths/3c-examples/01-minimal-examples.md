...
sidebar_position: 2
---
这页只给三个“最小能跑”的例子，不追求花哨功能，只追求**验证闭环可见**。你把它们跑通之后，就能知道 proof、public inputs、验证事件、以及（必要时）聚合结果在工程里各自扮演什么角色。每个例子都刻意控制规模，让你在 10–20 分钟内看到结果。

为了避免“只是跑通但不理解”，每个例子都回答同一组工程问题：

1) 我到底在证明什么？
2) 哪些输入是公开的，哪些必须留在私有侧？
3) 结果要在应用侧消费，还是需要走聚合？

你可以把它们理解成“最小可复用脚手架”，把核心输入输出抽出来，换成你自己的业务数据即可。
...
> 💡 Tip: 如果你只想确认 zkVerify 的验证闭环，先跑 Hash preimage。它的结构最简单，最容易定位错误。
...
你可以把它想成“开锁验证”：验证方只看锁上的编号，私有输入是钥匙本体。只要钥匙能打开锁，就证明你知道原像。
...
> ⚠️ Warning: 不要把 `secret` 当成 public input 提交。这样等于公开了你要隐藏的东西，证明就失去意义。
...
把它看成“名单验收单”：root 是名单的摘要，path 是你从叶子到 root 的路线。验证方只需要 root 和你的路径就能验证成员关系。
...
> 💡 Tip: 路径方向错是最常见的 bug。先打印每层的 `cur`，看是在哪一层开始偏离。
...
> ⚠️ Warning: 顺序错了看起来像“proof 总是失败”，其实是输入映射错了。
...
这五步不是为了“写流程”，而是为了防止你在步骤 3/4 卡住时不知道该回头查哪一环。
...
> 💡 Tip: 调试时先把输入打印成“可读结构”，再看 proof/验证。很多时候问题不在算法，而在输入的排列或编码。

这一页的核心不是让你记住三个例子，而是让你拥有一个“最小可运行”起点。你跑通之后，下一节会给出生产风格的例子，把这些最小骨架扩展成真实应用场景。
